...
class TestCacheDecorator(TestCase):
    ...
    def test_cache_params(self):
        resp = self.client.get(
            "/testhello/test_params/?dkjha=asjdaks&params_b="
            "alsdl&ksjdkasjd=askdjakshd&asdhghashd=asjdkajs")
        self.assertEqual(force_text(resp.content), "test params")

    def test_decorator_with_pk(self):
        resp = self.client.get(
            "/testhello/23/test_with_pk/"
            "?dkjha=asjdaks&params_b=alsdl&ksjdkasjd=askdjakshd&asdhghashd=asjdkajs")
        self.assertEqual(force_text(resp.content), "test params")

    def test_cache_with_pk(self):
        resp = self.client.get(
            "/testhello/23/test_cache_with_pk/"
            "?dkjha=asjdaks&params_b=alsdl&ksjdkasjd=askdjakshd&asdhghashd=asjdkajs")
    ...
    def test_cache_with_list(self):
        resp = self.client.get(
            "/testhello/test_cache_with_list/"
            "?dkjha=asjdaks&params_b=alsdl&ksjdkasjd=askdjakshd&asdhghashd=asjdkajs")
        con_1 = force_text(resp.content)
        resp = self.client.get(
            "/testhello/test_cache_with_list/"
    ...
    def test_cache_without_followseed(self):
        resp = self.client.get(
            "/testhello/test_cache_no_follow/"
            "?dkjha=asjdaks&params_b=alsdl&ksjdkasjd=askdjakshd&asdhghashd=asjdkajs")
        con_1 = force_text(resp.content)
        resp = self.client.get(
            "/testhello/test_cache_no_follow/"
    ...
    def test_update_seed_version_object(self):
        ...
        # 其它对象是缓存的
        resp = self.client.get(
            "/testhello/24/test_cache_with_pk/"
            "?dkjha=asjdaks&params_b=alsdl&ksjdkasjd=askdjakshd&asdhghashd=asjdkajs")
