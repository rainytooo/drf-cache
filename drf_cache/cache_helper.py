...
class CacheVersion(object):
    ...
    def cache_is_new(self, cache_key, resource_name, resource_id, resource_type):
        ...
        logger.debug('seed version key: %s, seed version value: %s' % (seed_version_key, seed_version))

        cache_version_key = self.calculate_cache_version_key(cache_key, resource_name, resource_id, resource_type)
        cache_version = self.get_version_by_key(cache_version_key)
        logger.debug('cache version key: %s, cache version value: %s' % (cache_version_key, cache_version))
